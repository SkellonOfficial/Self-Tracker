...
{
    [BepInPlugin("org.skellon.gorillatag.selftrackerutils", "SelfTrackerUtils", "1.0.0")]
    public class NotifiLib : BaseUnityPlugin
    {
        private void Init()
        {
            ...
            Testtext.font = Font.CreateDynamicFontFromOSFont("VIVL Rail", 15);
        ...
        private void FixedUpdate()
        {
            ...
            if (Testtext.text != "")
            {
                ...
                if (NotificationDecayTimeCounter > NotificationDecayTime)
                {
                    ...
                    Notifilines = Testtext.text.Split(Environment.NewLine.ToCharArray()).Skip<string>(1).ToArray();
        ...
        {
            string text = "";
            foreach (string text2 in NotifiText.text.Split(Environment.NewLine.ToCharArray()).Skip<string>(amount).ToArray<string>())
            {
                if (text2 != "")
                {
                    text = text + text2 + "\n";
